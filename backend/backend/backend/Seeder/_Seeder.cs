    ...
    public static class Seeder
    {
        public static void Seed(DbContext context)
        {
            if (!context.Set<Category>().Any() && !context.Set<ProductImage>().Any() && !context.Set<Product>().Any())
            {
                ...
                ProductImage superGuyImage1 = new ProductImage { ImageAlt = "Image de produit 2: SuperGuy", Order = 1, ImageData = GetImageFromFile("superguy-0.png"), ContentType = "image/png" };
